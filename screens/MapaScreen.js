...
export default function MapaScreen({ navigation }) {
  ...
  const larguraVaga = (Dimensions.get('window').width - 40 - (NUM_COLS - 1) * 10) / NUM_COLS;
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#1e1e1e' }}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <Text style={styles.subtitle}>Vagas ocupadas de acordo com motos cadastradas</Text>

        <View style={styles.grid}>
          {vagas.map((ocupada, i) => (
            <View
              key={i}
              style={[styles.vaga, ocupada ? styles.ocupada : styles.livre, { width: larguraVaga, height: larguraVaga }]}
            >
